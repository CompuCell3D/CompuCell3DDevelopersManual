Building A Growth Steppable In The Main CC3D Code
==================================================
...
.. note::
    ...
    This case study generates the steppable inside the main CC3D source tree, under ``core/CompuCell3D/steppables``. The next examples show the same kind of task in DeveloperZone, which is usually the better location for custom extension modules. See :doc:`building_growth_steppable_in_developer_zone` for the DeveloperZone version of this workflow.
...
The ``step`` and ``start`` functions are the first functions we will modify. The generated ``step`` function already contains useful iteration code, but the ``start`` function will be rewritten. Let's take a look:
...
.. code-block:: xml
    ...
    <GrowthRate CellType="1">1.0</GrowthRate>
